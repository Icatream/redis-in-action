...
import ria.lettuce.pojo.Article;
...
public class Chapter01 extends BaseChapter {
    ...
    public Mono<Long> articleVote(long articleId, long userId) {
        ...
        String a = Key01.v_ARTICLE(articleId);
        return comm.zscore(Key01.ZS_TIME, a)
          .filter(d -> d >= cutOff)
          .then(comm.sadd(Key01.S_VOTED(articleId), Key01.v_USER(userId))
            .filter(i -> i == 1)
            .then(comm.zincrby(Key01.ZS_SCORE, VOTE_SCORE, a)
              .then(comm.hincrby(a, Key01.f_VOTES, 1))));
    }
    ...
    public Mono<Long> postArticle(Article article) {
        return comm.incr(Key01.ARTICLE)
          .flatMap(id -> {
              article.setId(id);
              long now = Instant.now().getEpochSecond();
              article.setTime(now);
              String voted = Key01.S_VOTED(id);
              String a = Key01.v_ARTICLE(id);
              return comm.sadd(voted, Key01.v_USER(article.getPosterId()))
                .then(comm.expire(voted, Duration.ofDays(7).getSeconds()))
                .then(putArticle(article))
                .then(comm.zadd(Key01.ZS_SCORE, now + VOTE_SCORE, a))
                .then(comm.zadd(Key01.ZS_TIME, now, a))
                .thenReturn(id);
          });
    }

    public Flux<Article> getArticles(int page) {
        return getArticles(page, Key01.ZS_SCORE);
    }
    ...
    public Flux<Long> changeGroup(long articleId, List<Long> addGroupIds, List<Long> removeGroupIds) {
        ...
        return Flux.fromIterable(addGroupIds)
          .flatMap(groupId -> comm.sadd(Key01.S_GROUP(groupId), a))
          .concatWith(Flux.fromIterable(removeGroupIds)
            .flatMap(groupId -> comm.srem(Key01.S_GROUP(groupId), a)));
    }

    public Flux<Article> getGroupArticles(long groupId, int page) {
        return getGroupArticles(groupId, page, Key01.ZS_SCORE);
    }

    public Flux<Article> getGroupArticles(long groupId, int page, String order) {
        String group = Key01.S_GROUP(groupId);
    ...
    private Mono<String> putArticle(Article article) {
        ...
        return comm.hmset(Key01.v_ARTICLE(article.getId()), map);
    }
