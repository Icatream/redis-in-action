...
public class Chapter08 extends BaseChapter {
    ...
    public Mono<String> createUser(String login, String name) {
        ...
        String lockName = Key08.v_USER(lLogin);
        //acquire lock, if failed user-side retry
        return Chapter06.acquireLockWithTimeout(comm, lockName)
          .flatMap(lockId -> comm.hget(Key08.USER, lLogin)
            .map(s -> Tuples.of(false, s))
            //user id no exist, create user
            .switchIfEmpty(comm.incr(Key08.USER_ID)
              .map(String::valueOf)
              .flatMap(id -> Mono.zip(comm.hset(Key08.USER, lLogin, id),
                comm.hmset(Key08.H_USER(id), generateUser(login, id, name)))
    ...
    public Mono<String> createStatus(String userId, String message, Map<String, String> data) {
        String userKey = Key08.H_USER(userId);
        return comm.hget(userKey, "login")
          .flatMap(login -> comm.incr(Key08.STATUS_ID)
            .map(String::valueOf)
            .flatMap(id -> {
                ...
                return Mono.zip(comm.hmset(Key08.H_STATUS(id), data),
    ...
    public Flux<Map<String, String>> getStatusMessages(String userId, String timeLine, long start, long end) {
        return comm.zrevrange(timeLine + BaseKey.SEPARATOR + userId, start, end)
          .map(Key08::H_STATUS)
    ...
    public Mono<Void> followUser(String userId, String otherUserId) {
        String fKey1 = Key08.Z_FOLLOWING(userId);
        String fKey2 = Key08.Z_FOLLOWERS(otherUserId);
        long now = ZonedDateTime.now().toEpochSecond();
        String homeKey = Key08.Z_HOME(userId);
        ...
        return comm.zscore(fKey1, otherUserId)
          ...
          .switchIfEmpty(Mono.zip(comm.zadd(fKey1, now, otherUserId)
              .flatMap(following -> comm.hincrby(Key08.H_USER(userId), Key08.FOLLOWING, following)),
            comm.zadd(fKey2, now, userId)
              .flatMap(followers -> comm.hincrby(Key08.H_USER(otherUserId), Key08.FOLLOWERS, followers)))
            .then(comm.zrevrangeWithScores(Key08.Z_PROFILE(otherUserId), 0, HOME_TIMELINE_SIZE)
              .collectList()
        ...
        return comm.zscore(fKey1, otherUserId)
          .flatMap(d -> Mono.zip(comm.zrem(fKey1, otherUserId)
            ...
            comm.zrem(fKey2, userId)
              .flatMap(followers -> comm.hincrby(Key08.H_USER(otherUserId), Key08.FOLLOWERS, -followers)))
            .then(comm.zrevrange(Key08.Z_PROFILE(otherUserId), 0, HOME_TIMELINE_SIZE - 1)
              .collectList()
              .filter(list -> list.size() > 0)
              .flatMap(list -> comm.zrem(Key08.Z_HOME(userId), list.toArray(new String[0])))));
    }
    ...
    public Mono<String> postStatus(String userId, String message, Map<String, String> data) {
        return createStatus(userId, message, data)
          .flatMap(id -> comm.hget(Key08.H_STATUS(id), "posted")
            .map(Double::valueOf)
            .flatMap(posted -> comm.zadd(Key08.Z_PROFILE(userId), posted, id)
    ...
    public Mono<Long> syndicateStatus(String userId, ScoredValue<String> post, long offset) {
        String followers = Key08.Z_FOLLOWERS(userId);
    ...
    public Mono<Boolean> deleteStatus(String userId, String statusId) {
        String key = Key08.H_STATUS(statusId);
        return Chapter06.acquireLockWithTimeout(comm, key, 1)
          .flatMap(lock -> comm.hget(key, "uid")
            ...
            .flatMap(s -> Mono.zip(comm.del(key),
              comm.zrem(Key08.Z_PROFILE(userId), statusId),
              comm.zrem(Key08.Z_HOME(userId), statusId),
              comm.hincrby(Key08.H_USER(userId), "posts", -1)))
